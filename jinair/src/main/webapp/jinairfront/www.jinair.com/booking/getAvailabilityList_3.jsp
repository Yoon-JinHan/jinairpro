...
<%
		...
		int count = adultPaxCount + childPaxCount + infantPaxCount;
		...
		//adultPaxCount+childPaxCount+infantPaxCount
		for(int i = 1; i <= adultPaxCount+childPaxCount+infantPaxCount; i++ ){
			//System.out.println(request.getParameter("name"+i));
			names.add(request.getParameter("name"+i));
			sexs.add(request.getParameter("sex"+i));
			births.add(request.getParameter("birth"+i));
			nationalitys.add(request.getParameter("nationality"+i));
			mIds.add(request.getParameter("mId"+i));
			discounts.add(request.getParameter("discount"+i));
			parents.add(request.getParameter("parent"+i));
		}
		
%>

<form id="registerform"  action="getAvailabilityList.jsp"> <!-- ******************************** -->
		<input type="hidden" 	id="s1_time" 			name="s1_time" 				value="${ param.s1_time }"/>
		<input type="hidden" 	id="b1_type" 			name="b1_type" 				value="${ param.b1_type }"/>
		<input type="hidden" 	id="s2_time" 			name="s2_time" 				value="${ param.s2_time }"/>
		<input type="hidden" 	id="b2_type" 			name="b2_type" 				value="${ param.b2_type }"/>
		
		<input type="hidden" 	id="fare1" 		  		name="fare1" 				value="${ param.fare1 }">
		<input type="hidden"	id="fuel1" 		  		name="fuel1" 				value="${ param.fuel1 }">
		<input type="hidden"	id="tax1" 	 			name="tax1" 				value="${ param.tax1 }">
		<input type="hidden"	id="fare2" 	 			name="fare2" 				value="${ param.fare2 }">
		<input type="hidden"	id="fuel2" 				name="fuel2" 				value="${ param.fuel2 }">
		<input type="hidden"	id="tax2" 		  		name="tax2" 				value="${ param.tax2 }">
		
		<input type="hidden" 	id="totalfare" 			name="totalfare" 			value="${ param.totalfare }">
		<input type="hidden" 	id="totalfuel" 			name="totalfuel" 			value="${ param.totalfuel }">
		<input type="hidden" 	id="totaltax" 			name="totaltax" 			value="${ param.totaltax }">
		<input type="hidden" 	id="totalprice" 		name="totalprice" 			value="${ param.totalprice }">
		
		<input type="hidden" name="type" value="온라인"/>
		<input type="hidden" name="channel" value="온라인"/>
		
		<input type="hidden" name="email" value="${ param.email }"/>
		<input type="hidden" name="tel" value="${ param.tel }"/>
		
		<c:forEach var="name" items="<%= names %>" varStatus="status">
			<input type="hidden" name="name${ status.count }" value="${ name }"/>
		</c:forEach>
		<c:forEach var="sex" items="<%= sexs %>" varStatus="status">
			<input type="hidden" name="sex${ status.count }" value="${ sex }"/>
		</c:forEach>
		<c:forEach var="birth" items="<%= births %>" varStatus="status">
			<input type="hidden" name="birth${ status.count }" value="${ birth }"/>
		</c:forEach>
		<c:forEach var="nationality" items="<%= nationalitys %>" varStatus="status">
			<input type="hidden" name="nationality${ status.count }" value="${ nationality }"/>
		</c:forEach>
		<c:forEach var="mId" items="<%= mIds %>" varStatus="status">
			
			<input type="hidden" name="mId${ status.count }" value="${ mId }"/>
		</c:forEach>
		<c:forEach var="discount" items="<%= discounts %>" varStatus="status">
			<input type="hidden" name="discount${ status.count }" value="${ discount }"/>
		</c:forEach>
		<c:forEach var="parent" items="<%= parents %>" varStatus="status">
			<input type="hidden" name="parents${ status.count }" value="${ parent }"/>
		</c:forEach>
...
<div class="step">
	...
	<ol>
		...
		<c:set var="origin2" value="${ param.origin2 }"></c:set>	
		
		<form id="registerform2"> <!-- ******************************** -->
				<table id="service">
					<tr>
						<td width="40%" style="border-bottom: 2px solid black; border-right: 2px solid black;">여정1 - ${ param.origin1 } -> ${ param.destination1 }</td>
						<c:if test="${ origin2 != '' }">
							<td width="40%" style="border-bottom: 2px solid black;">여정2 - ${ param.origin2 } -> ${ param.destination2 }</td>
						</c:if>
					</tr>
					<tr>
						<td style="border-right: 2px solid black; ">
							<c:forEach var="name" items="<%= names %>" varStatus="status">
								<br>
								<h3><span style="font-size: 25px; font-weight: 30px;">${ name }</span> 님</h3> 				 
								<table>
									<tr>
										<td height="50px" width="100px">사전좌석지정 </td>
										<td><input type="text" name="seat1-${ status.count }" id="seat1-${ status.count }"/></td>
									</tr>
									<tr>
										<td height="50px" width="100px">초과수화물</td>
										<td><input type="text" name="weight1-${ status.count }" id="weight1-${ status.count }"/></td>
									</tr>
									<tr>
										<td height="50px" width="100px">기내식</td>
										<td><input type="text" name="food1-${ status.count }" id="food1-${ status.count }"/></td>
									</tr>
									<tr> <td><button type="button" id="addservice1-${ status.count }">부가서비스 등록</button></td></tr>
								</table>
								<hr />
							</c:forEach>
						</td>
						<c:if test="${ origin2 != '' }">
							<td>
								...
								<table>
									<tr>
										<td height="50px" width="100px">사전좌석지정 </td>
										<td><input type="text" name="seat2-${ status.count }" id="seat2-${ status.count }"/></td>
									</tr>
									<tr>
										<td height="50px" width="100px">초과수화물</td>
										<td><input type="text" name="weight2-${ status.count }" id="weight2-${ status.count }"/></td>
									</tr>
									<tr>
										<td height="50px" width="100px">기내식</td>
										<td><input type="text" name="food2-${ status.count }" id="food2-${ status.count }"/></td>
									</tr>
									<tr> <td><button type="button" id="addservice2-${ status.count }">부가서비스 등록</button></td></tr>
								</table>
								<hr />
							</c:forEach>
							</td>
		...
		<script>
			var count = 0;
			var len = <%= count %>;
			const seat = [new Array(len).fill(0),new Array(len).fill(0)];
			const baggage = [new Array(len).fill(0),new Array(len).fill(0)];
			const food = [new Array(len).fill(0),new Array(len).fill(0)];
			var totalprice = Number( "${ param.totalprice }".replace(",","") );
			$("table#service button").on("click", function() {
				var index = $(this).attr("id").substr(-4).replace("e","");
				var index1 = Number(index.substr(0,1));
				var index2 = Number(index.substr(-2).replace("-",""));
				//alert(index1);
				
				
				var params = "origin="+$("#origin"+index1).val()+"&destination="+$("#destination"+index1).val()+"&travelDate="+$("#travelDate"+index1).val()+" "+$("#s"+index1+"_time").val()
							+"&seat="+$("#seat"+index).val()+"&weight="+$("#weight"+index).val()+"&food="+$("#food"+index).val();
				//alert(params);
				$.ajax({
					url:"checkservice.do", 
					dataType:"json", 		
					...
					success: function(data, textStatus, jpXHR) {
						var seatprice = data.service.seat.price;
						var baggageprice = data.service.baggage.price;
						var foodprice = data.service.food.price;
						...
						seat[index1-1][index2-1] = seatprice;
						baggage[index1-1][index2-1] = baggageprice;
						food[index1-1][index2-1] = foodprice;
						
						for(var i = 0; i < 2; i++ ){
							for(var j = 0; j < len; j++){
								seattotal += seat[i][j];
								baggagetotal += baggage[i][j];
								foodtotal += food[i][j];
							}
						}
						/* 	
						...
						if(count == 0){
							if(seatprice != 0){
								$(".mySrmy .content .scr #basketSSRSummary").append("<dt>사전좌석지정</dt><dd id='seatTaxSum'>" + seattotal.toLocaleString() + "</dd>");
							}
							if(baggageprice != 0)$(".mySrmy .content .scr #basketSSRSummary").append("<dt>초과수화물</dt><dd id='baggageTaxSum'>" + baggagetotal.toLocaleString()  + "</dd>");
							if(foodprice != 0)$(".mySrmy .content .scr #basketSSRSummary").append("<dt>기내식</dt><dd id='foodTaxSum'>" + foodtotal.toLocaleString()  + "</dd>");
						} else {	
							if( $(".mySrmy .content .scr #basketSSRSummary dd#seatTaxSum").text() != "")  $(".mySrmy .content .scr #basketSSRSummary #seatTaxSum").text(seattotal.toLocaleString());
							else if(seatprice != 0) $(".mySrmy .content .scr #basketSSRSummary").append("<dt>사전좌석지정</dt><dd id='seatTaxSum'>" + seattotal.toLocaleString() + "</dd>");
							if( $(".mySrmy .content .scr #basketSSRSummary dd#baggageTaxSum").text() != "")  $(".mySrmy .content .scr #basketSSRSummary #baggageTaxSum").text(baggagetotal.toLocaleString());
							else if(baggageprice != 0) $(".mySrmy .content .scr #basketSSRSummary").append("<dt>초과수화물</dt><dd id='baggageTaxSum'>" + baggagetotal.toLocaleString()  + "</dd>");
							if( $(".mySrmy .content .scr #basketSSRSummary dd#foodTaxSum").text() != "")  $(".mySrmy .content .scr #basketSSRSummary #foodTaxSum").text(foodtotal.toLocaleString());
							else if(foodprice != 0) $(".mySrmy .content .scr #basketSSRSummary").append("<dt>기내식</dt><dd id='foodTaxSum'>" + foodtotal.toLocaleString()  + "</dd>");
						}
						 
						$("#totalCharge").text( (totalprice+seattotal+baggagetotal+foodtotal).toLocaleString() );
			...
			function next() {
				var count = "${ param.origin2 }" == "" ? 1 : 2;
				for(var i = 1; i <= <%= count %>; i++){
					for(var j = 1; j <= count; j++){
						$("#registerform").append("<input type='hidden' id='seat"+j+"-"+i+"' name='seat"+j+"-"+i+"' value='"+$("#registerform2 #seat"+j+"-"+i).val()+"'/>");
						$("#registerform").append("<input type='hidden' id='weight"+j+"-"+i+"' name='weight"+j+"-"+i+"' value='"+$("#registerform2 #weight"+j+"-"+i).val()+"'/>");
						$("#registerform").append("<input type='hidden' id='food"+j+"-"+i+"' name='food"+j+"-"+i+"' value='"+$("#registerform2 #food"+j+"-"+i).val()+"'/>");
					}
				}
				
				$("#registerform").append("<input type='hidden' id='seatTaxSum' name='seatTaxSum' value='"+$(".mySrmy .content .scr #basketSSRSummary #seatTaxSum").text()+"'/>");
				$("#registerform").append("<input type='hidden' id='baggageTaxSum' name='baggageTaxSum' value='"+$(".mySrmy .content .scr #basketSSRSummary #baggageTaxSum").text()+"'/>");
				$("#registerform").append("<input type='hidden' id='foodTaxSum' name='foodTaxSum' value='"+$(".mySrmy .content .scr #basketSSRSummary #foodTaxSum").text()+"'/>");
				
				$("#registerform #totalprice").val($("#totalCharge").text());
				
				
				var form = $("#registerform");
				form.attr("action","getAvailabilityList_4.jsp");
		...
		<div class="btnArea" id="nextbtnWrap">
			...
			<a href="#" onclick="next(); return false" class="btnTypeA sizeL" data-click-area="여정-3_탑승객정보" data-click-name="다음">다음</a>
		</div>
...
<div id="totalSrmy">
	...
	<div class="price">
		...
		<p class="btnNext">
					...
					<a href="#" onclick="return next();" data-click-area="여정-3_항공권선택" data-click-name="다음" class="btnTypeA">
...
<script>
	(function() {
		$(".content .scr dl dd#flightTaxSum").text("${ param.totalfare }");
		$(".content .scr dl dd#fuelTaxSum").text("${ param.totalfuel }");
		$(".content .scr dl dd#basicTaxSum").text("${ param.totaltax }");
		$("#totalCharge").text("${ param.totalprice }");
	})()
</script>
