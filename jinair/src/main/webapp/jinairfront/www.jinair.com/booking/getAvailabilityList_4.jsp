...
<%
		...
		String sseatTaxSum = request.getParameter("seatTaxSum").replace(",","");
		String sbaggageTaxSum = request.getParameter("baggageTaxSum").replace(",","");
		String sfoodTaxSum = request.getParameter("foodTaxSum").replace(",","");
		
		int seatTaxSum = Integer.parseInt( sseatTaxSum == "" ? "0" : sseatTaxSum  );
		int baggageTaxSum = Integer.parseInt( sbaggageTaxSum == "" ? "0" : sbaggageTaxSum );
		int foodTaxSum = Integer.parseInt( sfoodTaxSum == "" ? "0" : sfoodTaxSum );
		
		String servicePrice = String.format("%,d",seatTaxSum+baggageTaxSum+foodTaxSum);
		...
		ArrayList<String> weights1 = new ArrayList<>();
		ArrayList<String> foods1 = new ArrayList<>();
		ArrayList<String> seats2 = new ArrayList<>();
		ArrayList<String> weights2 = new ArrayList<>();
		...
		for(int i = 1; i <= count; i++ ){
			...
			seats1.add(request.getParameter("seats1-"+i));
			weights1.add(request.getParameter("weights1-"+i));
			foods1.add(request.getParameter("foods1-"+i));
			seats2.add(request.getParameter("seats2-"+i));
			weights2.add(request.getParameter("weights2-"+i));
			foods2.add(request.getParameter("foods2-"+i));
			String [] list = {request.getParameter("name"+i),request.getParameter("sex"+i),request.getParameter("birth"+i),request.getParameter("nationality"+i)};
			totals.add(list);
...
<form id="registerform"  action="getAvailabilityList.jsp"> <!-- ******************************** -->
		...
		<input type="hidden" 	id="b_count" 		name="b_count" 			value="<%= count %>">
		...
		<c:forEach var="name" items="<%= names %>" varStatus="status"> <input type="hidden" name="name${ status.count }" value="${ name }"/> </c:forEach>
		<c:forEach var="sex" items="<%= sexs %>" varStatus="status"> <input type="hidden" name="sex${ status.count }" value="${ sex }"/> </c:forEach>
		<c:forEach var="birth" items="<%= births %>" varStatus="status"> <input type="hidden" name="birth${ status.count }" value="${ birth }"/> </c:forEach>
		<c:forEach var="nationality" items="<%= nationalitys %>" varStatus="status"> <input type="hidden" name="nationality${ status.count }" value="${ nationality }"/> </c:forEach>
		<c:forEach var="mId" items="<%= mIds %>" varStatus="status"> <input type="hidden" name="mId${ status.count }" value="${ mId }"/> </c:forEach>
		<c:forEach var="discount" items="<%= discounts %>" varStatus="status"> <input type="hidden" name="discount${ status.count }" value="${ discount }"/> </c:forEach>
		<c:forEach var="parent" items="<%= parents %>" varStatus="status"> <input type="hidden" name="parents${ status.count }" value="${ parent }"/> </c:forEach>
		
		<c:forEach var="seat1" items="<%= seats1 %>" varStatus="status"> <input type="hidden" name="seats1-${ status.count }" value="${ seat1 }"/> </c:forEach>
		<c:forEach var="weight1" items="<%= weights1 %>" varStatus="status"> <input type="hidden" name="weights1-${ status.count }" value="${ weight1 }"/> </c:forEach>
		<c:forEach var="food1" items="<%= foods1 %>" varStatus="status"> <input type="hidden" name="foods1-${ status.count }" value="${ food1 }"/> </c:forEach>
		<c:forEach var="seat2" items="<%= seats2 %>" varStatus="status"> <input type="hidden" name="seats2-${ status.count }" value="${ seat2 }"/> </c:forEach>
		<c:forEach var="weight2" items="<%= weights2 %>" varStatus="status"> <input type="hidden" name="weights2-${ status.count }" value="${ weight2 }"/> </c:forEach>
		<c:forEach var="food2" items="<%= foods2 %>" varStatus="status"> <input type="hidden" name="foods2-${ status.count }" value="${ food2 }"/> </c:forEach>
...
function showMulti(){
	...
	if ($("#quickModify .multiArea li:first-child ~li").length > 0) {
		...
		<h1 class="typeA">결제</h1>
	
	
<script>
	function showDetail(obj) {
	    $(obj).parents(".head").next(".detail").stop().slideToggle(150);
	    $(obj).toggleClass("on");
	}
</script>
...
<div class="step">
	...
	<ol>
		...
		<li class="on"><strong>5</strong> <span>결제</span></li>
	</ol>
</div>
		
		<div id="listSchedule"></div>	
		
		<h2 class="typeA">탑승객 정보 </h2>
		<table class="bbsList">
			<caption>구분, 성명, 생년월일, 국적 순으로 구성된 탑승객 정보 안내표입니다.</caption>
			<colgroup>
				<col style="width:25%;">
				<col style="width:15%;">
				<col style="width:25%;">
			...
			<thead>
				<tr>
					<th scope="col">성명</th> <!-- 성명 -->
					<th scope="col">구분</th> <!-- 구분 -->
					<th scope="col">생년월일</th> <!-- 생년월일 -->
				
					<th scope="col">국적</th> <!-- 국적 -->
				
				</tr>
			</thead>
			<tbody id="boader">
				<c:forEach var="total" items="<%= totals %>" varStatus="status">
					<tr>
						<td>${ total[0] } </td>
						<td>${ total[1] } </td>
						<td>${ total[2] } </td>
		...
		<div class="flightInfo">
			<p class="tripType">
				...
				<div class="tripSrmy">
					...
					<!-- 170830 수정 -->
					<p class="trip">
						<strong>${ param.origin1 }</strong>
						[출발 ]
						${ param.destination1 } ${ param.s1_time }"
					</p>
					<span class="ico single">편도</span>
					<!-- 170830 수정 -->
					<p class="trip">
						<strong>${ param.destination1 }</strong> [도착 ]
						수정 2022-01-24(월) 21:45
					</p>
		...
		<ul class="detailInfo">
			<li>
				<div class="head">
					...
					<span class="unit">KRW</span>
					${ param.totalfare }
					<a href="#" onclick="showDetail(this); return false" class="icoFold">자세히 보기 </a>
				</div>
				<div class="detail" style="display: none">
						...
						<h3>
							...
							:
							${ param.origin1 }-${ param.destination1 }
						...
						<h3>
							...
							:
							${ param.origin2 }-${ param.destination2 }
			...
			<li>
				<div class="head">
					...
					<span class="unit">KRW </span>
					${ param.totaltax }
					<a href="#" onclick="showDetail(this); return false" class="icoFold">자세히보기</a>
				</div>
				<div class="detail" style="display: none">
		...
		<h2 class="typeA">결제정보</h2>
				<div class="totalPrice">
					...
					<p class="mark">
						<img src="//images.jinair.com/images/common/ico_minus.png" alt="-">
					</p>
					<div class="sec">
						<em>할인금액 </em>
						<p>
							<span class="unit" >KRW</span> <strong id="dicountAmt">0</strong>
						</p>
					</div>
					<p class="mark">
						<img src="//images.jinair.com/images/common/ico_equal.png" alt="=">
					</p>
					<div class="sec total">
						<em>최종 결제금액 </em>
						<p>
							<span class="unit">KRW</span>
							<strong id="paymentAmt">${ param.totalprice }</strong>
						</p>
					</div>
				</div>
		...
		<div class="btnArea" id="nextbtnWrap">
			...
			<a href="#" onclick="next(); return false" class="btnTypeA sizeL" data-click-area="여정-3_탑승객정보" data-click-name="결제">결제</a>
		</div>
...
<script>
			...
			function next() {
				...
				form.attr("action","getAvailabilityList_3.jsp");
