...
$(document).ready(function() {
	...
	$(".cal a").click(function(){
		var url = "/jinair/jinairfront/www.jinair.com/ageCalculator";
...
function getCityAirport(){
	...
	$.ajax({
		type : "POST",
		url : "/jinair/jinairfront/www.jinair.com/booking/getAirportListJson.json",
		data : data,
...
function confirmLayerYes() {
	...
	switch (arrCountry) {
		case "KOR":
			/* 해외출발 -> 국내도착 */
			switch ( depCountry ) {
				case "JPN": noteTarget = "JP"; break;
				case "PHL": noteTarget = "PH"; break;
				case "VNM": noteTarget = "VN"; break;
				case "MNP": noteTarget = "SP"; break;
				case "AUS": noteTarget = "AU"; break;
				case "GUM": noteTarget = "US"; break;
				case "USA": noteTarget = "US"; break;
				default: noteTarget=""; break; }
			break;
		case "JPN":
			if (multiCheck) {
				if ($(".itinerary.multiCourse").find(".sec.destination>p>a>strong").length>1) {
					noteTarget = "JP";
				}
			}
			else {
				if ($('input[name=tripType]:checked').val() !='' && $('input[name=tripType]:checked').val() != "OW") {
					noteTarget = "JP";
				}
			}
			break;
	...
	if(itin.indexOf(';GUMICN;') !== -1) {
		noteTarget = 'GUM';
	} else if(itin.indexOf(';BKKICN;') !== -1) {
		noteTarget = 'BKK';
	} else if(itin.indexOf(';CNXICN;') !== -1) {
		noteTarget = 'CNX';
	}

	// 출/도착지에 따른 안내 팝업 표출 대상일 때
	if(noteTarget.length > 0) {
		showPopupLayer("/jinair/jinairfront/www.jinair.com/popup/noteDestination?destination=" + noteTarget);
		return false;
	}
	...
	setTimeout(function(){ $('#registerform').submit(); }, 100);
}
...
function setMemberNum(self) {
	...
	if (!bonusCoupon || bonusCoupon.apoCd.split("|")[0].split(",")[0] == "ALL") {
		if ($(self).parents(".multiCourse").is(":visible")) { // 다구간
			showBooking($(self).parents(".multiCourse").find(".course li:first-child .departure p"));
		} else { // 왕복, 편도
			showBooking($(self).parents(".itinerary").find(".departure>p"));
		}
	}
}
...
function showCalendar(self, title, bindYN) {
	...
	showPopupLayer("/jinair/jinairfront/www.jinair.com/popup/getBestFares?title=" + encodeURIComponent(title)
...
function setItineraryInfo() {
	...
	if ($('#tripType').val() == "MC" || $("#registerform #pnrNumber").val() != "") { // 다구간
		$(".multiCourse").show().siblings(".itinerary").hide();
...
function submitItinerary() {
	// 다구간 또는 예약 변경일 때
	var tripType = $('input[name=tripType]:checked').val();
	if(tripType === "MC" || $("#registerform #pnrNumber").val() != "") {
	...
	case "KOR":
		...
		case "CHN":
			if(langCd == "KOR" || langCd == "CHN") {
				if( (depCity == "CJU" && arrCity == "XIY") || (depCity == "XIY" && arrCity == "CJU") || 
					(depCity == "CJU" && arrCity == "PVG") ){
					noteTarget = "CN"; break;
				}
			}
		default: noteTarget=""; break; }
	...
	case "CHN":
		...
		// 인천(ICN) - 칭타오(TAO) 팝업
		if( depCity =="ICN" && arrCity == "TAO"){
			if(langCd == "KOR" || langCd == "CHN" || langCd == "CHT" || langCd == "ENG") {
				noteTarget = "TAO"; break;
			}
		}

		// 인천(ICN)김포(GMP) - 시안(XIY) 팝업
		if(langCd == "KOR" || langCd == "CHN") {
			if( depCity == "ICN" || depCity == "GMP" ){
				if(arrCity == "XIY") {
					noteTarget = "ICNGMPXIY"; break;
				}
			}
		}
	...
	case "TWN": // 타이베이, 타오위안
		//noteTarget = "CHN"; break;	
	case "THA":
		// 태국(방콕, 푸켓), 필리핀(세부, 클락, 보라카이/칼리보), 베트남(다낭, 하노이)
		if( arrCity == "BKK" || arrCity == "HKT" || arrCity == "CEB" ||
			arrCity == "CRK" || arrCity == "KLO" || arrCity == "DAD" || arrCity == "HAN"){
			//noteTarget = "THA"; break;
		}
	case "LAO":
		//noteTarget = "THA"; break;
	case "MYS":	
	...
	// 출/도착지에 따른 안내 팝업 표출 대상일 때
	if(noteTarget.length > 0) {
		// 원주-제주, 군산-제주 노선 선택 시 안내 문구 추가 
		var cjuWjuKuvLine = "N";
		if((arrCity == "CJU" && depCity == "WJU") || (arrCity == "WJU" && depCity == "CJU") ||
				(arrCity == "KUV" && depCity == "CJU") || (arrCity == "CJU" && depCity == "KUV")){
			cjuWjuKuvLine = "Y"
		}

		showPopupLayer("/jinair/jinairfront/www.jinair.com/popup/noteDestination?destination=" + noteTarget + "&cjuWjuKuvLine=" + cjuWjuKuvLine);
		return false;
	}
...
function jsClickBonusCouponBtn(isLogin){
	if ( isLogin ){
		showPopupLayer('/jinair/jinairfront/www.jinair.com/bonus');
	} else {
